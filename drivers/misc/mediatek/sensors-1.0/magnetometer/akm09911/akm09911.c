...
#define pr_fmt(fmt) "<AKM09911> " fmt
...
/*----------------------------------------------------------------------------*/
static int akm09911_i2c_probe(struct i2c_client *client,
			      const struct i2c_device_id *id);
static int akm09911_i2c_remove(struct i2c_client *client);
static int akm09911_i2c_detect(struct i2c_client *client,
...
/* Set measure mode */
	...
	if ((akm_device == 0x05) ||
		...
		if (ret < 0) {
		   ...
		   0x10) { /* ak09915 & ak09916c & ak09916d & ak09918 */
		akm_fuse[0] = 0x80;
		akm_fuse[1] = 0x80;
		akm_fuse[2] = 0x80;
	}
...
static int AKECS_AxisInfoToPat(const uint8_t axis_order[3],
			       const uint8_t axis_sign[3], int16_t *pat)
{
	/* check invalid input */
...
{
	u8 _bIndex = 0;
	u8 _baRegMap[] = {0x00, 0x01, 0x02, 0x03, 0x10, 0x11, 0x12,
			  0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x30,
			  0x31, 0x32, 0x33, 0x60, 0x61, 0x62};
	/* u8  _baRegValue[20]; */
	ssize_t _tLength = 0;
	char tmp[2] = {0};
	int ret = 0;
...
{
	int idx, err = 0;
	int num = (int)(sizeof(akm09911_attr_list) /
			sizeof(akm09911_attr_list[0]));
...
static int akm09911_batch(int flag, int64_t samplingPeriodNs,
			  int64_t maxBatchReportLatencyNs)
{
	...
	pr_debug("akm09911 mag set delay = (%d) ok.\n", value);
	return 0;
}
...
static int akm09911_factory_enable_sensor(bool enabledisable,
